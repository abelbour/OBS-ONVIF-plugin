#!/usr/bin/env python3
"""Drives tests/unit/discovery_live_test.cpp against the mock server.

Covers the M2→M3 discovery bridge's contact resolution end-to-end without
multicast:
  seed    -- mock HTTP on 127.0.0.1; a unicast Probe registers the camera in
             the live table and persists it (cameras.json in a temp config dir).
  rehome  -- mock re-bound to a new loopback host (127.0.0.2); the bridge
             seeds the persisted camera, detects the xaddr move, and reports
             it through the onMoved callback with the new stream URI.

usage: run_discovery_bridge_test.py <discovery_live_test binary>
"""
import os
import shutil
import socket
...
def main():
    if len(sys.argv) < 2 or not os.path.exists(sys.argv[1]):
        print("usage: run_discovery_bridge_test.py "
              "<discovery_live_test binary>", file=sys.stderr)
        return 2
    binary = sys.argv[1]
    cfgdir = tempfile.mkdtemp(prefix="obs-onvif-disctest-")
    rc = 0
    try:
    ...
    if rc == 0:
        print("ALL DISCOVERY BRIDGE SCENARIOS PASSED")
    return rc
