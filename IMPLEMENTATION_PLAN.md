...
- Day-night/BLC quirks cross-check against Hikvision + Dahua fixtures. **Remaining (needs real hardware).**
- DHCP-sack: Hello-listener reliability at scale (socket reuse, TTL bookkeeping) under repeated adds. **Done:** the discovery bridge classifies every datagram (`DiscoveryMsgType`) and applies it cheaply — a Hello refreshes presence for a known camera with zero SOAP, a Bye marks it offline immediately, only ProbeMatches (or an unknown/new-address Hello) resolve fully; the loop socket bumps `SO_RCVBUF` and drains datagram bursts in a bounded batch so a Hello flood is not starved behind slow resolution. `discovery_live` gained a `presence` phase (Hello-on → Bye-off, both against a dead XAddr proving no SOAP) and `ws_discovery` asserts message-type parsing.
- ABI consumer field test: a small real consumer (or a staging build of Advanced Scene Switcher behavior) covering move/stop, full preset lifecycle, and scene-binding set/clear. **Done:** `tests/aclink` is a C consumer that resolves the real exported symbol table and drives camera list, full preset lifecycle (save/list/goto/rename/delete/current), move/stop, and scene-binding set/clear against the mock; `config_live` covers the camera-config ABI read/write set.
- Packaging: zip artifacts with `data/locale/en-US.ini`, ABI header, README, user guide; publish to OBS forum resources. **Done (CI):** the release zip already ships the DLL, locale, and `obs-onvif.h`; `Package-Windows.ps1` now also bundles `LICENSE`, `THIRD_PARTY_NOTICES.md`, `README.md`, and `docs/USER_GUIDE.md`. **Remaining:** publishing the release to the OBS forum resources (manual step).
- GPLv3: add the required license text + attribution for ODM (GPLv2) and any vendored MIT headers (nlohmann, TinyXML2 zlib). **Done:** `LICENSE` is the full GPL-3.0 text; `THIRD_PARTY_NOTICES.md` attributes obs-plugintemplate (GPL-2.0), ODM (GPL-2.0, ported logic), OBS Studio (GPL-2.0), TinyXML2 (zlib), nlohmann/json (MIT), Qt6 (LGPL-3.0, runtime via OBS), and the Windows SDK APIs.
